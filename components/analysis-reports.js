import { inspectTree as inspect, referenceIndex, mtextPlain } from '../packages/dxf-inspector/index.mjs';
  ...
  class AnalysisReports {
    ...
    install() {
      ...
      if (w) {
        ...
        this.detachWorkspace = w.onDispose(() => this.dispose());
      }
    }
    statistics() {
    ...
    dispose(){if(this.disposed)return;this.disposed=true;this.detachWorkspace?.();this.diagnosticGeneration++;this.abort.abort();for(const {view}of this.views.values())view.dispose();this.views.clear();this.diagnosticResults.clear();for(const[name,method]of this.originals)this.app[name]=method;}
  }
