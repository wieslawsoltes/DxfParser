/* Pure, bounded presentation models. Aggregation never changes report rows.
 * Keys retain their original type; labels are never used as object identity. */
(function (root, factory) {
    const api = factory();
    if (typeof module === 'object' && module.exports)
        module.exports = api;
    root.DxfAnalysisVisualModel = api;
})(globalThis, function () {
    'use strict';
    const label = v => v === true ? 'Yes' : v === false ? 'No' : v == null || v === '' ? '(empty)' : String(v);
    const numeric = v => typeof v === 'number' && Number.isFinite(v);
    const keyOf = v => typeof v + ':' + String(v ?? '');
    function aggregate(rows, group, measure = -1) {
    ...
    function histogram(rows, column, count = 10) {
    ...
    function matrix(rows, x, y, maxX = 8, maxY = 8) {
    ...
    function byteStats(rows, column) {
    ...
    function compareRows(columns, baseline, selected) {
    ...
    return { label, numeric, aggregate, top, histogram, matrix, byteStats, compareRows };
});
