...
class DrawingNavigationTests(h.MultipleDrawingTests):
    ...
    def test_nav_11_locked_view_rejects_tiling_atomically(self):
        ...
        self.page.evaluate('() => {window.before=w.manager.SaveLayout(); }');self.command('RENDERTILE grid')
        self.assertJS('w.manager.SaveLayout()===before && a.cad.log.textContent.includes("movable")')
