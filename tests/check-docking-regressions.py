...
def main():
    commands=[['node','scripts/check-skia-distribution.js'],
              ['node','--test',*[str(p.relative_to(ROOT)) for p in sorted((ROOT/'packages/dxf-skia/tests').glob('*.test.js'))],'tests/skia-native.test.mjs','tests/skia-performance-native.test.mjs','tests/skia-recovery.test.mjs','tests/skia-webgpu-abi.test.mjs'],
